...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseServer
      ...
      .select('id, first_name, last_name, email, phone, expertise, years_experience, status, reviewed_at, created_at')
    ...
    if (q) {
      const like = `%${q}%`
      query = query.or(
        `first_name.ilike.${like},last_name.ilike.${like},email.ilike.${like},phone.ilike.${like},expertise.ilike.${like}`
      ) as any
    }
