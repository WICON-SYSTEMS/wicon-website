...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Handle CV attachments using bucket+path structure
    ...
    const photoFields = ['applicant_photo_url', 'photo_url', 'photo', 'image_url', 'applicant_photo']
    ...
    const cvFields = ['cv_url', 'resume_url', 'cv', 'resume', 'document_url']
    for (const field of cvFields) {
      if (data[field] && isStorageKey(data[field])) {
        const signedUrl = await generateSignedUrl('applications-cv', data[field])
        if (signedUrl) {
          enrichedData[`${field}_signed`] = signedUrl
        }
      }
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to fetch internship application' },
      { status: 500 }
    )
  }
}
