...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseServer
      ...
      .select('id, full_name, email, phone, position, availability, status, reviewed_at, created_at')
    ...
    if (q) {
      const like = `%${q}%`
      query = query.or(
        `full_name.ilike.${like},email.ilike.${like},phone.ilike.${like},position.ilike.${like}`
      ) as any
    }
